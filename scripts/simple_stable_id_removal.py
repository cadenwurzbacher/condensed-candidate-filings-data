#!/usr/bin/env python3
"""
Simple removal of stable ID generation methods from state cleaner files.
This script ONLY removes the specific methods that generate stable IDs,
without touching any other code structure or indentation.
"""
...
def remove_stable_id_methods(file_path):
    ...
    # Remove ONLY the _generate_stable_ids method and its calls
...
def main():
    ...
    print("🧹 Simple removal of stable ID generation methods...")
    print("=" * 60)
    ...
    for file_path in state_cleaner_files:
        if remove_stable_id_methods(file_path):
            modified_count += 1
    
    print()
    print("=" * 60)
    print(f"🎉 Stable ID method removal complete!")
