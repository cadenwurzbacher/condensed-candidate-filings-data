#!/usr/bin/env python3
"""
Minimal removal of stable ID generation methods from state cleaner files.
This script ONLY removes the specific methods that generate stable IDs,
without touching any other code structure, indentation, or formatting.
"""
...
def remove_stable_id_methods_minimal(file_path):
    """Remove only stable ID generation methods from a single file."""
    ...
    # Remove ONLY the _generate_stable_ids method completely
...
def main():
    ...
    print("🧹 Minimal removal of stable ID generation methods...")
    print("=" * 60)
    ...
    for file_path in state_cleaner_files:
        if remove_stable_id_methods_minimal(file_path):
            modified_count += 1
    
    print()
    print("=" * 60)
    print(f"🎉 Minimal stable ID method removal complete!")
