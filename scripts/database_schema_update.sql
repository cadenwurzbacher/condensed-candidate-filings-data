-- Database Schema Update Script (FINAL FIXED VERSION)
...
SELECT 
    ...
    CASE 
        WHEN filing_date IS NULL OR filing_date = 'NULL' OR filing_date = 'null' OR LENGTH(TRIM(filing_date)) = 0 THEN NULL
        ELSE filing_date::DATE
    END as filing_date, 
    CASE 
        WHEN election_date IS NULL OR election_date = 'NULL' OR election_date = 'null' OR LENGTH(TRIM(election_date)) = 0 THEN NULL
        ELSE election_date::DATE
