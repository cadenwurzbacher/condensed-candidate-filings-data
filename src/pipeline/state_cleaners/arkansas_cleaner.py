#!/usr/bin/env python3
"""
Arkansas State Data Cleaner

This module contains functions to clean and standardize Arkansas political candidate data
...
class ArkansasCleaner:
    """Handles cleaning and standardization of Arkansas political candidate data."""
    ...
    def clean_arkansas_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Clean and standardize Arkansas candidate data according to final schema.
        
        Args:
            df: Raw Arkansas candidate data DataFrame
            
        Returns:
            Cleaned DataFrame conforming to final schema
        """
        logger.info(f"Starting Arkansas data cleaning for {len(df)} records...")
        ...
        logger.info(f"Arkansas data cleaning completed. Final shape: {cleaned_df.shape}")
        return cleaned_df
    ...
    def _process_election_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """Process election year and type from filing date and context."""
        logger.info("Processing election data...")
        
        # Arkansas data is for 2024 elections
        df['election_year'] = 2024
        
        # Determine election type - Arkansas typically has Primary and General
        # Since this appears to be filing data, we'll default to Primary
        # but this could be refined based on filing dates
        df['election_type'] = "Primary"  # Default, could be refined
    ...
    def _process_office_and_district(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def process_office_district(office_str: str) -> Tuple[str, Optional[str]]:
            ...
            if "representative" in office_str.lower() or "congress" in office_str.lower():
                return "US Representative", None
            
            # Handle US Senate
            if "senate" in office_str.lower() and "state" not in office_str.lower():
                return "US Senate", None
            
            # Handle State Senate
            if "state senate" in office_str.lower():
                return "State Senate", None
            
            # Handle State House/Representative
            if "state house" in office_str.lower() or "state representative" in office_str.lower():
            ...
            # Handle Commissioner
            if "commissioner" in office_str.lower():
    ...
    def _process_candidate_names(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Apply name cleaning
        df['candidate_name'] = df['Candidate Name'].apply(clean_name)
    ...
    def _clean_contact_info(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Arkansas data doesn't have phone, email, address, or website
        # Set these to None
        df['phone'] = None
        df['email'] = None
        df['address'] = None
...
def clean_arkansas_candidates(input_file: str, output_file: str = None, output_dir: str = DEFAULT_OUTPUT_DIR) -> pd.DataFrame:
    """
    Main function to clean Arkansas candidate data.
    ...
    """
    # Load the data
    logger.info(f"Loading Arkansas data from {input_file}...")
    df = pd.read_excel(input_file)
    
    # Initialize cleaner with output directory
    cleaner = ArkansasCleaner(output_dir=output_dir)
    
    # Clean the data
    cleaned_df = cleaner.clean_arkansas_data(df)
...
if __name__ == "__main__":
    ...
    if available_files:
        ...
        cleaned_data = clean_arkansas_candidates(input_file, output_dir=output_dir)
