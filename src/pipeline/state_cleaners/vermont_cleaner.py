#!/usr/bin/env python3
"""
Vermont State Data Cleaner

This module contains functions to clean and standardize Vermont political candidate data
...
class VermontCleaner:
    """Handles cleaning and standardization of Vermont political candidate data."""
    ...
    def clean_vermont_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Clean and standardize Vermont candidate data according to final schema.
        
        Args:
            df: Raw Vermont candidate data DataFrame
            
        Returns:
            Cleaned DataFrame conforming to final schema
        """
        logger.info(f"Starting Vermont data cleaning for {len(df)} records...")
        ...
        logger.info(f"Vermont data cleaning completed. Final shape: {cleaned_df.shape}")
        return cleaned_df
    ...
    def _process_election_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """Process election year and type from Vermont data."""
        logger.info("Processing election data...")
        
        # Vermont data is from 2024, with both Primary (August 13) and General (November 5) elections
        # Extract from filename or set based on data context
        df['election_year'] = 2024
        
        # Determine election type based on the data structure
        # Vermont has both primary and general elections in the same file
        # We'll need to determine this from the context or set a default
        df['election_type'] = "Primary"  # Default, can be updated based on specific logic
    ...
    def _process_office_and_district(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def process_office_district(office_str: str, district_str: str, name_str: str) -> Tuple[str, Optional[str]]:
            ...
            district_str = str(district_str).strip() if pd.notna(district_str) else None
            
            # Handle US President
            if "PRESIDENT" in office_str.upper():
                return "US President", None
            
            # Handle US Representative
            if "REPRESENTATIVE" in office_str.upper() and "UNITED STATES" in office_str.upper():
                return "US Representative", "At Large"
            
            # Handle Governor
            if "GOVERNOR" in office_str.upper():
                return "Governor", None
            
            # Handle Lieutenant Governor
            if "LIEUTENANT GOVERNOR" in office_str.upper():
                return "Lieutenant Governor", None
            
            # Handle Attorney General
            if "ATTORNEY GENERAL" in office_str.upper():
                return "Attorney General", None
            
            # Handle Auditor
            if "AUDITOR" in office_str.upper():
                return "Auditor", None
            
            # Handle High Bailiff (Vermont-specific)
            if "HIGH BAILIFF" in office_str.upper():
                return "High Bailiff", district_str
            
            # Handle State Senate
            if "SENATE" in office_str.upper():
                return "State Senate", district_str
            
            # Handle State House
            if "HOUSE" in office_str.upper():
                return "State House", district_str
            
            # Handle other offices (keep as is)
            return office_str, district_str
        
        # Apply office and district processing
        office_results = df.apply(lambda row: process_office_district(row['office'], row['district'], row['candidate_name']), axis=1)
    ...
    def _clean_contact_info(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Clean phone numbers - combine day and evening phone
        def clean_phone(day_phone: str, evening_phone: str) -> str:
            # Try day phone first, then evening phone
            for phone in [day_phone, evening_phone]:
        ...
        # Enhanced address parsing and cleaning for Vermont
        ...
        # Apply cleaning
        # Structural cleaner now consolidates phones into 'phone'; fall back second arg safely if present
        fallback_col = 'Evening Phone' if 'Evening Phone' in df.columns else None
        if fallback_col:
            df['phone'] = df.apply(lambda row: clean_phone(row['phone'], row[fallback_col]), axis=1)
        else:
            df['phone'] = df['phone'].apply(lambda p: clean_phone(p, None))
        ...
        # Map address components only if legacy columns exist; otherwise keep structural outputs
        ...
        if 'Town Of Residence' in df.columns:
    ...
    def _add_required_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['original_filing_date'] = pd.NA  # Not available in Vermont data
...
def clean_vermont_candidates(input_file: str, output_file: str = None, output_dir: str = DEFAULT_OUTPUT_DIR) -> pd.DataFrame:
    """
    Main function to clean Vermont candidate data.
    ...
    """
    # Load the data
    logger.info(f"Loading Vermont data from {input_file}...")
    
    # Vermont data has a complex structure, so we need to read it carefully
    df = pd.read_excel(input_file, header=None)
    
    # Find the actual data rows (skip header rows)
    # Look for the row that contains "Contest" as the first column
    data_start_row = None
    for idx, row in df.iterrows():
        if pd.notna(row[0]) and str(row[0]).strip() == "Contest":
            data_start_row = idx
            break
    
    if data_start_row is None:
        raise ValueError("Could not find data start row in Vermont file")
    ...
    # Clean the data
    cleaned_df = cleaner.clean_vermont_data(data_rows)
...
if __name__ == "__main__":
    ...
    # Example usage - use the Vermont file specifically
    vermont_files = [f for f in available_files if 'vermont' in f.lower()]
    if vermont_files:
        input_file = vermont_files[0]  # Use the Vermont file
        ...
        cleaned_data = clean_vermont_candidates(input_file, output_dir=output_dir)
        print(f"\nCleaned {len(cleaned_data)} records")
        print(f"Columns: {cleaned_data.columns.tolist()}")
        print(f"Data saved to: {output_dir}/")
    else:
        print("No Vermont files found. Available files:")
        for i, file_path in enumerate(available_files, 1):
            print(f"  {i}. {os.path.basename(file_path)}")
        print("\nTo process a specific file, modify the script or call clean_vermont_candidates() directly.")
