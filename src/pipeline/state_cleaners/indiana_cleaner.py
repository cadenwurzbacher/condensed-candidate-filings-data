#!/usr/bin/env python3
"""
Indiana State Data Cleaner

This module contains functions to clean and standardize Indiana political candidate data
...
class IndianaCleaner:
    """Handles cleaning and standardization of Indiana political candidate data."""
    ...
    def clean_indiana_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Clean and standardize Indiana candidate data according to final schema.
        
        Args:
            df: Raw Indiana candidate data DataFrame
            
        Returns:
            Cleaned DataFrame conforming to final schema
        """
        logger.info(f"Starting Indiana data cleaning for {len(df)} records...")
        ...
        logger.info(f"Indiana data cleaning completed. Final shape: {cleaned_df.shape}")
        return cleaned_df
    ...
    def _process_election_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Indiana data has Year and Election columns
    ...
    def _process_office_and_district(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def process_office_district(office_str: str) -> Tuple[str, Optional[str]]:
            ...
            senate_match = re.match(r'.*SENATE.*DISTRICT\s*(\d+)', office_str, re.IGNORECASE)
            if senate_match:
                district = senate_match.group(1)
                return "State Senate", district
            
            # Handle State House districts
            house_match = re.match(r'.*HOUSE.*DISTRICT\s*(\d+)', office_str, re.IGNORECASE)
            if house_match:
    ...
    def _process_full_name_displays(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def clean_name(name_str: str) -> str:
            ...
            # Indiana names have format like "(Indiana's Own) Dana Black" or "(Renie) Irene Coffman"
            # Extract the actual name part after the parentheses
            if '(' in name_str and ')' in name_str:
                # Find the closing parenthesis and extract everything after it
                end_paren = name_str.find(')')
                if end_paren != -1:
                    actual_name = name_str[end_paren + 1:].strip()
                    return actual_name
    ...
    def _parse_names(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, row in df.iterrows():
            ...
            if pd.isna(name) or not name:
                ...
                df.at[idx, 'full_name_display'] = None
                continue
            
            # For Indiana data, parse the cleaned candidate name
    ...
    def _clean_contact_info(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Indiana data doesn't have contact info columns, so set to None
        df['phone'] = pd.NA
        df['email'] = pd.NA
        df['address'] = pd.NA
        df['website'] = pd.NA
...
def clean_indiana_candidates(input_file: str, output_file: str = None, output_dir: str = DEFAULT_OUTPUT_DIR) -> pd.DataFrame:
    """
    Main function to clean Indiana candidate data.
    ...
    """
    # Load the data
    logger.info(f"Loading Indiana data from {input_file}...")
    ...
    cleaner = IndianaCleaner(output_dir=output_dir)
    
    # Clean the data
    cleaned_df = cleaner.clean_indiana_data(df)
...
if __name__ == "__main__":
    ...
    # Find Indiana file specifically
    indiana_file = None
    for file_path in available_files:
        if 'indiana' in os.path.basename(file_path).lower():
            indiana_file = file_path
            break
    
    if indiana_file:
        print(f"\nFound Indiana file: {os.path.basename(indiana_file)}")
        output_dir = DEFAULT_OUTPUT_DIR
        
        print(f"Output directory: {output_dir}")
        
        # Clean the Indiana data and save to the output directory
        cleaned_data = clean_indiana_candidates(indiana_file, output_dir=output_dir)
        print(f"\nCleaned {len(cleaned_data)} records")
        print(f"Columns: {cleaned_data.columns.tolist()}")
        print(f"Data saved to: {output_dir}/")
    else:
        print("No Indiana file found. Processing first available file instead.")
        ...
        cleaned_data = clean_indiana_candidates(input_file, output_dir=output_dir)
        print(f"\nCleaned {len(cleaned_data)} records")
        print(f"Columns: {cleaned_data.columns.tolist()}")
        print(f"Data saved to: {output_dir}/")
