#!/usr/bin/env python3
"""
Illinois State Data Cleaner

This module contains functions to clean and standardize Illinois political candidate data
...
class IllinoisCleaner:
    """Handles cleaning and standardization of Illinois political candidate data."""
    ...
    def clean_illinois_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Clean and standardize Illinois candidate data according to final schema.
        
        Args:
            df: Raw Illinois candidate data DataFrame
            
        Returns:
            Cleaned DataFrame conforming to final schema
        """
        logger.info(f"Starting Illinois data cleaning for {len(df)} records...")
        ...
        # Step 6: Parse filing dates
        cleaned_df = self._parse_filing_dates(cleaned_df)
        ...
        logger.info(f"Illinois data cleaning completed. Final shape: {cleaned_df.shape}")
        return cleaned_df
    ...
    def _process_office_and_district(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def process_office_district(office_str: str) -> Tuple[str, Optional[str]]:
            ...
            if "PRESIDENT AND VICE PRESIDENT" in office_str or "PRESIDENT" in office_str:
                return "US President", None
            
            # Handle US Representative (Illinois uses "1ST CONGRESS", "2ND CONGRESS", etc.)
            congress_match = re.search(r'(\d+)(?:ST|ND|RD|TH) CONGRESS', office_str, re.IGNORECASE)
            if congress_match:
                district = congress_match.group(1)
            ...
            # Handle Illinois Democratic/Republican Delegates
            delegate_match = re.search(r'(\d+)(?:ST|ND|RD|TH)?\s+(DEMOCRATIC|REPUBLICAN)\s+DELEGATE', office_str, re.IGNORECASE)
            if delegate_match:
                district = delegate_match.group(1)
                return "State Delegate", district
            
            # Handle State Senate districts
            senate_match = re.search(r'SENATE DISTRICT (\d+)', office_str, re.IGNORECASE)
            if senate_match:
                district = senate_match.group(1)
                return "State Senate", district
            
            # Handle State House districts
            house_match = re.search(r'HOUSE DISTRICT (\d+)', office_str, re.IGNORECASE)
            if house_match:
    ...
    def _process_candidate_names(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def clean_name(name_str: str, office_str: str) -> str:
            ...
            if "US PRESIDENT" in office_str or "US VICE PRESIDENT" in office_str or "PRESIDENT AND VICE PRESIDENT" in office_str:
    ...
    def _standardize_parties(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        party_mapping = {
            ...
            'write-in': 'Write-in',
            'other': 'Other',
            # Illinois-specific party mappings
        ...
        def standardize_party(party_str: str) -> str:
            ...
            party_str_clean = str(party_str).strip()
            party_lower = party_str_clean.lower()
            
            # First try exact match (case-sensitive)
            if party_str_clean in party_mapping:
                return party_mapping[party_str_clean]
            
            # Then try lowercase match
            if party_lower in party_mapping:
                return party_mapping[party_lower]
    ...
    def _parse_filing_dates(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'Date Filed' in df.columns:
            def parse_filing_date(date_str):
                ...
                try:
                    # Handle Illinois date format like "11/19/20242:27 PM"
                    # Remove time portion and parse date
                    date_part = str(date_str).split(' ')[0]  # Get just the date part
                    if '/' in date_part:
                        # Parse MM/DD/YYYY format
                        month, day, year = date_part.split('/')
                        # Handle cases where year might have extra digits
                        if len(year) > 4:
                            year = year[:4]
...
def clean_illinois_candidates(input_file: str, output_file: str = None, output_dir: str = DEFAULT_OUTPUT_DIR) -> pd.DataFrame:
    """
    Main function to clean Illinois candidate data.
    ...
    """
    # Load the data
    logger.info(f"Loading Illinois data from {input_file}...")
    df = pd.read_excel(input_file)
    
    # Initialize cleaner with output directory
    cleaner = IllinoisCleaner(output_dir=output_dir)
    
    # Clean the data
    cleaned_df = cleaner.clean_illinois_data(df)
...
if __name__ == "__main__":
    ...
    # Example usage - use the first available Illinois file
    illinois_files = [f for f in available_files if 'illinois' in os.path.basename(f).lower()]
    if illinois_files:
        input_file = illinois_files[0]  # Use the first available Illinois file
        ...
        cleaned_data = clean_illinois_candidates(input_file, output_dir=output_dir)
        print(f"\nCleaned {len(cleaned_data)} records")
        print(f"Columns: {cleaned_data.columns.tolist()}")
        print(f"Data saved to: {output_dir}/")
    else:
        print("No Illinois input files available for processing")
