#!/usr/bin/env python3
"""
Oklahoma State Data Cleaner

This module contains functions to clean and standardize Oklahoma political candidate data
...
class OklahomaCleaner:
    """Handles cleaning and standardization of Oklahoma political candidate data."""
    ...
    def clean_oklahoma_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Clean and standardize Oklahoma candidate data according to final schema.
        
        Args:
            df: Raw Oklahoma candidate data DataFrame
            
        Returns:
            Cleaned DataFrame conforming to final schema
        """
        logger.info(f"Starting Oklahoma data cleaning for {len(df)} records...")
        ...
        logger.info(f"Oklahoma data cleaning completed. Final shape: {cleaned_df.shape}")
        return cleaned_df
    ...
    def _clean_contact_info(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Oklahoma data doesn't have contact info, so set to None
        df['phone'] = None
        df['email'] = None
        df['address'] = None
        df['website'] = None
...
def clean_oklahoma_candidates(input_file: str, output_file: str = None, output_dir: str = DEFAULT_OUTPUT_DIR) -> pd.DataFrame:
    """
    Main function to clean Oklahoma candidate data.
    ...
    """
    # Load the data
    logger.info(f"Loading Oklahoma data from {input_file}...")
    df = pd.read_excel(input_file)
    
    # Initialize cleaner with output directory
    cleaner = OklahomaCleaner(output_dir=output_dir)
    
    # Clean the data
    cleaned_df = cleaner.clean_oklahoma_data(df)
...
if __name__ == "__main__":
    ...
    # Example usage - specifically target Oklahoma file
    oklahoma_file = None
    for file_path in available_files:
        if 'oklahoma' in os.path.basename(file_path).lower():
            oklahoma_file = file_path
            break
    
    if oklahoma_file:
        input_file = oklahoma_file
        ...
        cleaned_data = clean_oklahoma_candidates(input_file, output_dir=output_dir)
        print(f"\nCleaned {len(cleaned_data)} records")
        print(f"Columns: {cleaned_data.columns.tolist()}")
        print(f"Data saved to: {output_dir}/")
    else:
        print("No Oklahoma file found for processing")
