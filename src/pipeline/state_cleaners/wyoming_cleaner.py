#!/usr/bin/env python3
"""
Wyoming State Data Cleaner

This module contains functions to clean and standardize Wyoming political candidate data
...
def list_available_input_files(input_dir: str = DEFAULT_INPUT_DIR) -> List[str]:
    ...
    for file in os.listdir(input_dir):
        if file.endswith(('.xlsx', '.xls')) and 'wyoming' in file.lower() and not file.startswith('~$'):
            excel_files.append(os.path.join(input_dir, file))
    
    return sorted(excel_files)

class WyomingCleaner:
    """Handles cleaning and standardization of Wyoming political candidate data."""
    ...
    def clean_wyoming_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Clean and standardize Wyoming candidate data according to final schema.
        
        Args:
            df: Raw Wyoming candidate data DataFrame
            
        Returns:
            Cleaned DataFrame conforming to final schema
        """
        logger.info(f"Starting Wyoming data cleaning for {len(df)} records...")
        ...
        logger.info(f"Wyoming data cleaning completed. Final shape: {cleaned_df.shape}")
        return cleaned_df
    ...
    def _process_election_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """Process election year and type from filename and office column."""
    ...
    def _clean_contact_info(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Enhanced address parsing and cleaning for Wyoming
    ...
    def _add_required_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['original_filing_date'] = df['filing_date'].copy() if 'filing_date' in df.columns else pd.NA
...
def clean_wyoming_candidates(input_file: str, output_file: str = None, output_dir: str = DEFAULT_OUTPUT_DIR) -> pd.DataFrame:
    """
    Main function to clean Wyoming candidate data.
    ...
    """
    # Load the data
    logger.info(f"Loading Wyoming data from {input_file}...")
    df = pd.read_excel(input_file)
    
    # Initialize cleaner with output directory
    cleaner = WyomingCleaner(output_dir=output_dir)
    
    # Clean the data
    cleaned_df = cleaner.clean_wyoming_data(df)
...
if __name__ == "__main__":
    ...
    if available_files:
        ...
        cleaned_data = clean_wyoming_candidates(input_file, output_dir=output_dir)
