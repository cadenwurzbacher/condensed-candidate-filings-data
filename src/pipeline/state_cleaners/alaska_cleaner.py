...
class AlaskaCleaner:
    ...
    def clean_alaska_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Clean and standardize Alaska candidate data according to final schema.
        
        Args:
            df: Raw Alaska candidate data DataFrame
    ...
    def _process_election_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def extract_election_info(election_str: str) -> Tuple[Optional[int], Optional[str]]:
            ...
            # Handle special case for Xunaa Borough incorporation
            if "Incorporation Election of the Proposed Xunaa Borough" in election_str:
                return 2025, "Special"
        ...
        if invalid_incorporation.any():
            logger.info(f"Removing {invalid_incorporation.sum()} statewide/federal candidates from incorporation elections")
            df = df[~invalid_incorporation]
...
def clean_alaska_candidates(input_file: str, output_file: str = None, output_dir: str = DEFAULT_OUTPUT_DIR) -> pd.DataFrame:
    ...
    """
    # Load the data
    logger.info(f"Loading Alaska data from {input_file}...")
    df = pd.read_excel(input_file)
    
    # Initialize cleaner with output directory
    cleaner = AlaskaCleaner(output_dir=output_dir)
    
    # Clean the data
    cleaned_df = cleaner.clean_alaska_data(df)
...
if __name__ == "__main__":
    ...
    if available_files:
        ...
        cleaned_data = clean_alaska_candidates(input_file, output_dir=output_dir)
