#!/usr/bin/env python3
"""
Idaho State Data Cleaner

This module contains functions to clean and standardize Idaho political candidate data
...
def list_available_input_files(input_dir: str = DEFAULT_INPUT_DIR) -> List[str]:
    """
    List all available Excel and CSV files in the input directory.
...
class IdahoCleaner:
    """Handles cleaning and standardization of Idaho political candidate data."""
    ...
    def clean_idaho_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Clean and standardize Idaho candidate data according to final schema.
        
        Args:
            df: Raw Idaho candidate data DataFrame
            
        Returns:
            Cleaned DataFrame conforming to final schema
        """
        logger.info(f"Starting Idaho data cleaning for {len(df)} records...")
        ...
        logger.info(f"Idaho data cleaning completed. Final shape: {cleaned_df.shape}")
        return cleaned_df
    ...
    def _process_election_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """Process election year and type from data context."""
    ...
    def _process_office_and_district(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def process_office_district(position_str: str, dist_str: str, seat_str: str) -> Tuple[str, Optional[str]]:
            if pd.isna(position_str):
                return None, None
            
            position_str = str(position_str).strip()
            dist_str = str(dist_str).strip() if pd.notna(dist_str) else ""
            seat_str = str(seat_str).strip() if pd.notna(seat_str) else ""
            
            # Handle US President
            if "U.S. President" in position_str:
                return "US President", None
            
            # Handle US Representative
            if "U.S. Rep." in position_str:
                if dist_str:
                    return "US Representative", dist_str
                else:
                    return "US Representative", None
            
            # Handle State Senate
            if "State Senator" in position_str:
                if dist_str:
                    return "State Senate", dist_str
                else:
                    return "State Senate", None
            
            # Handle State Representative
            if "State Rep." in position_str:
                district = dist_str
                if seat_str:
                    district = f"{dist_str}-{seat_str}" if district else seat_str
    ...
    def _clean_contact_info(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Idaho data doesn't have contact info, so set to None
        df['phone'] = pd.NA
        df['email'] = pd.NA
        df['address'] = pd.NA
        df['website'] = pd.NA
...
def clean_idaho_candidates(input_file: str, output_file: str = None, output_dir: str = DEFAULT_OUTPUT_DIR) -> pd.DataFrame:
    """
    Main function to clean Idaho candidate data.
    ...
    """
    # Load the data
    logger.info(f"Loading Idaho data from {input_file}...")
    ...
    cleaner = IdahoCleaner(output_dir=output_dir)
    
    # Clean the data
    cleaned_df = cleaner.clean_idaho_data(df)
...
if __name__ == "__main__":
    ...
    # Example usage - use the first available Idaho file
    idaho_files = [f for f in available_files if 'idaho' in os.path.basename(f).lower()]
    if idaho_files:
        input_file = idaho_files[0]  # Use the first available Idaho file
        ...
        cleaned_data = clean_idaho_candidates(input_file, output_dir=output_dir)
        print(f"\nCleaned {len(cleaned_data)} records")
        print(f"Columns: {cleaned_data.columns.tolist()}")
        print(f"Data saved to: {output_dir}/")
    else:
        print("No Idaho input files available for processing")
