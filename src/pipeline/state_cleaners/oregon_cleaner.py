#!/usr/bin/env python3
"""
Oregon State Data Cleaner

This module contains functions to clean and standardize Oregon political candidate data
...
class OregonCleaner:
    """Handles cleaning and standardization of Oregon political candidate data."""
    ...
    def clean_oregon_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Clean and standardize Oregon candidate data according to final schema.
        
        Args:
            df: Raw Oregon candidate data DataFrame
            
        Returns:
            Cleaned DataFrame conforming to final schema
        """
        logger.info(f"Starting Oregon data cleaning for {len(df)} records...")
        ...
        logger.info(f"Oregon data cleaning completed. Final shape: {cleaned_df.shape}")
        return cleaned_df
    ...
    def _process_office_and_district(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def process_office_district(office_str: str) -> Tuple[str, Optional[str]]:
            ...
            # Handle US President
            if "President" in office_str and "State" not in office_str:
                return "US President", None
            
            # Handle US Representative
            if "Representative" in office_str and "State" not in office_str:
                # Extract district from office string
                district_match = re.search(r'(\d+)(?:st|nd|rd|th)?\s+District', office_str, re.IGNORECASE)
                if district_match:
            ...
            # Handle State Senate
            if "Senator" in office_str and "State" in office_str:
                # Extract district from office string
                district_match = re.search(r'(\d+)(?:st|nd|rd|th)?\s+District', office_str, re.IGNORECASE)
                if district_match:
            ...
            if "Representative" in office_str and "State" in office_str:
                # Extract district from office string
                district_match = re.search(r'(\d+)(?:st|nd|rd|th)?\s+District', office_str, re.IGNORECASE)
                if district_match:
...
def clean_oregon_candidates(input_file: str, output_file: str = None, output_dir: str = DEFAULT_OUTPUT_DIR) -> pd.DataFrame:
    """
    Main function to clean Oregon candidate data.
    ...
    """
    # Load the data
    logger.info(f"Loading Oregon data from {input_file}...")
    df = pd.read_excel(input_file)
    
    # Initialize cleaner with output directory
    cleaner = OregonCleaner(output_dir=output_dir)
    
    # Clean the data
    cleaned_df = cleaner.clean_oregon_data(df)
...
if __name__ == "__main__":
    ...
    # Example usage - specifically target Oregon file
    oregon_file = None
    for file_path in available_files:
        if 'oregon' in os.path.basename(file_path).lower():
            oregon_file = file_path
            break
    
    if oregon_file:
        input_file = oregon_file
        ...
        cleaned_data = clean_oregon_candidates(input_file, output_dir=output_dir)
        print(f"\nCleaned {len(cleaned_data)} records")
        print(f"Columns: {cleaned_data.columns.tolist()}")
        print(f"Data saved to: {output_dir}/")
    else:
        print("No Oregon file found for processing")
