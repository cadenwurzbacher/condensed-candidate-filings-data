#!/usr/bin/env python3
"""
Kansas State Data Cleaner

This module contains functions to clean and standardize Kansas political candidate data
...
class KansasCleaner:
    """Handles cleaning and standardization of Kansas political candidate data."""
    ...
    def clean_kansas_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Clean and standardize Kansas candidate data according to final schema.
        
        Args:
            df: Raw Kansas candidate data DataFrame
            
        Returns:
            Cleaned DataFrame conforming to final schema
        """
        logger.info(f"Starting Kansas data cleaning for {len(df)} records...")
        ...
        logger.info(f"Kansas data cleaning completed. Final shape: {cleaned_df.shape}")
        return cleaned_df
    ...
    def _process_office_and_district(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def process_office_district(office_str: str, district_str: str) -> Tuple[str, Optional[str]]:
            ...
            # Handle State Senate
            if "SENATE" in office_str.upper() and "STATE" in office_str.upper():
                return "State Senate", district_str if district_str else None
            
            # Handle State House
            if "HOUSE" in office_str.upper() and "REPRESENTATIVES" in office_str.upper():
    ...
    def _process_candidate_names(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'Candidate' in df.columns:
            df['candidate_name'] = df['Candidate'].apply(clean_name)
        else:
            ...
            if len(parts) >= 2:
                ...
                if len(first_middle) >= 1:
                    ...
                    if len(first_middle) >= 2:
                        ...
                        if len(first_middle) > 2:
                            additional_parts = []
                            for part in first_middle[2:]:
                                if self._should_treat_as_middle_name(part):
                                    additional_parts.append(part)
                            
                            if additional_parts:
                                if middle_name:
                                    middle_name = middle_name + ' ' + ' '.join(additional_parts)
                                else:
                                    middle_name = ' '.join(additional_parts)
    ...
    def _clean_contact_info(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Clean phone numbers - combine Home Phone and Cell Phone
        def clean_phone(home_phone: str, cell_phone: str) -> str:
            # Try cell phone first, then home phone
            for phone_str in [cell_phone, home_phone]:
        ...
        # Clean addresses - combine Home Address, Home City, Home Zip
        def clean_address(address_str: str, city_str: str, zip_str: str) -> str:
            parts = []
            
            if pd.notna(address_str):
                parts.append(str(address_str).strip())
            if pd.notna(city_str):
                parts.append(str(city_str).strip())
            if pd.notna(zip_str):
                parts.append(str(zip_str).strip())
    ...
    def _add_required_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        required_columns = [
            'id', 'stable_id', 'county', 'filing_date', 'election_date', 'facebook', 'twitter'
        ]
...
def clean_kansas_candidates(input_file: str, output_file: str = None, output_dir: str = DEFAULT_OUTPUT_DIR) -> pd.DataFrame:
    """
    Main function to clean Kansas candidate data.
    ...
    """
    # Load the data
    logger.info(f"Loading Kansas data from {input_file}...")
    df = pd.read_excel(input_file)
    
    # Initialize cleaner with output directory
    cleaner = KansasCleaner(output_dir=output_dir)
    
    # Clean the data
    cleaned_df = cleaner.clean_kansas_data(df)
...
if __name__ == "__main__":
    ...
    # Example usage - use the Kansas file specifically
    kansas_files = [f for f in available_files if 'kansas' in f.lower()]
    if kansas_files:
        input_file = kansas_files[0]  # Use the Kansas file
        ...
        cleaned_data = clean_kansas_candidates(input_file, output_dir=output_dir)
