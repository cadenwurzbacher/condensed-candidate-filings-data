#!/usr/bin/env python3
"""
South Dakota Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only South Dakota-specific column mapping logic remains.
"""
...
class SouthDakotaStructuralCleaner(BaseStructuralCleaner):
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            ...
            state_identifiers=['south_dakota', 'sd'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (South Dakota-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="South Dakota"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="South Dakota"),
