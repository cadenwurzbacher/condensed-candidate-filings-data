#!/usr/bin/env python3
"""
West Virginia Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only West Virginia-specific column mapping logic remains.
"""
...
class WestVirginiaStructuralCleaner(BaseStructuralCleaner):
    """
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            ...
            state_identifiers=['west_virginia', 'wv'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (West Virginia-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="West Virginia"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="West Virginia"),
