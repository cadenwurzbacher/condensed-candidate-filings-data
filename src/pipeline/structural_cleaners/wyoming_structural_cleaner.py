#!/usr/bin/env python3
"""
Wyoming Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only Wyoming-specific column mapping logic remains.
"""
...
class WyomingStructuralCleaner(BaseStructuralCleaner):
    """
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            state_name="Wyoming",
            state_identifiers=['wyoming', 'wy'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (Wyoming-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="Wyoming"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="Wyoming"),
