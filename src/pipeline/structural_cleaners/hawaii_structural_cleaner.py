#!/usr/bin/env python3
"""
Hawaii Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only Hawaii-specific column mapping logic remains.
"""
...
class HawaiiStructuralCleaner(BaseStructuralCleaner):
    """
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            state_name="Hawaii",
            state_identifiers=['hawaii', 'hi'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (Hawaii-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="Hawaii"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="Hawaii"),
