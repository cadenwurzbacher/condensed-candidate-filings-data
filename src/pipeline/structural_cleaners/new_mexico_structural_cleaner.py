...
class NewMexicoStructuralCleaner:
    """
    New Mexico Structural Cleaner - Phase 1 of new pipeline
    ...
    def clean(self) -> pd.DataFrame:
        """
        Extract structured data from New Mexico raw files
        
        Returns:
            pd.DataFrame: Structured data with consistent columns
        """
        logger.info("Starting New Mexico structural cleaning")
        
        # Find New Mexico raw files
        new_mexico_files = self._find_new_mexico_files()
        if not new_mexico_files:
            logger.warning("No New Mexico raw files found")
            return pd.DataFrame()
        
        # Process each file and combine
        all_records = []
        
        for file_path in new_mexico_files:
            try:
        ...
        if not all_records:
            logger.warning("No records extracted from New Mexico files")
            return pd.DataFrame()
        ...
        logger.info(f"New Mexico structural cleaning complete: {len(df)} records")
        return df
    
    def _find_new_mexico_files(self) -> list:
        """Find all New Mexico raw data files"""
        new_mexico_files = []
        
        if not os.path.exists(self.raw_dir):
            logger.warning(f"Raw data directory not found: {self.raw_dir}")
            return new_mexico_files
        
        # Look for New Mexico files (case insensitive)
        for file_path in Path(self.raw_dir).rglob("*"):
            if file_path.is_file():
                filename = file_path.name.lower()
                if 'new_mexico' in filename or 'new_mexico' in filename.replace('_', ''):
                    new_mexico_files.append(str(file_path))
        
        logger.info(f"Found {len(new_mexico_files)} New Mexico files: {new_mexico_files}")
        return new_mexico_files
    
    def _extract_from_file(self, file_path: str) -> list:
        """
        Extract structured data from a single New Mexico file
    ...
    def _extract_from_html(self, file_path: str) -> list:
        """Extract data from HTML file"""
        try:
            # Read HTML tables
            df_list = pd.read_html(file_path)
            if not df_list:
                logger.warning(f"No tables found in HTML file {file_path}")
                return []
            
            # Use the first table (usually the main data)
            df = df_list[0]
            logger.info(f"Read HTML file with {len(df)} rows and {len(df.columns)} columns")
    ...
    def _looks_like_candidate_data(self, df: pd.DataFrame) -> bool:
        ...
        candidate_indicators = ['contest', 'name', 'party', 'address', 'filing']
    ...
    def _is_valid_candidate_row(self, row: pd.Series) -> bool:
        ...
        # Check if we have at least a contest or name
        contest = str(row.get('Contest', '')).strip()
        first_name = str(row.get('First Name', '')).strip()
        last_name = str(row.get('Last Name', '')).strip()
        
        return (bool(contest and contest != 'nan') or
    ...
    def _extract_county(self, row: pd.Series) -> str:
        ...
        filing_county = str(row.get('Filing County', '')).strip()
        if filing_county and filing_county != 'nan':
            return filing_county
        return None
    ...
    def _extract_state(self, row: pd.Series) -> str:
        ...
        if state and state != 'nan':
            return state
        return 'New Mexico'  # Default to New Mexico
    ...
    def _extract_filing_date(self, row: pd.Series) -> str:
        ...
        if pd.notna(filing_datetime):
            # Convert to string format
            if hasattr(filing_datetime, 'strftime'):
                return filing_datetime.strftime('%Y-%m-%d')
            else:
                return str(filing_datetime)
        return None
    ...
    def _extract_election_type(self, row: pd.Series) -> str:
        ...
        # New Mexico typically has primary and general elections
