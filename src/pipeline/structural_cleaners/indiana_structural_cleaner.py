...
class IndianaStructuralCleaner:
    """
    Indiana Structural Cleaner - Phase 1 of new pipeline
    ...
    def clean(self) -> pd.DataFrame:
        """
        Extract structured data from Indiana raw files
        
        Returns:
            pd.DataFrame: Structured data with consistent columns
        """
        logger.info("Starting Indiana structural cleaning")
        
        # Find Indiana raw files
        indiana_files = self._find_indiana_files()
        if not indiana_files:
            logger.warning("No Indiana raw files found")
            return pd.DataFrame()
        
        # Process each file and combine
        all_records = []
        
        for file_path in indiana_files:
            try:
        ...
        if not all_records:
            logger.warning("No records extracted from Indiana files")
            return pd.DataFrame()
        ...
        logger.info(f"Indiana structural cleaning complete: {len(df)} records")
        return df
    
    def _find_indiana_files(self) -> list:
        """Find all Indiana raw data files"""
        indiana_files = []
        
        if not os.path.exists(self.raw_dir):
            logger.warning(f"Raw data directory not found: {self.raw_dir}")
            return indiana_files
        
        # Look for Indiana files (case insensitive)
        for file_path in Path(self.raw_dir).rglob("*"):
            if file_path.is_file():
                filename = file_path.name.lower()
                if 'indiana' in filename:
                    indiana_files.append(str(file_path))
        
        logger.info(f"Found {len(indiana_files)} Indiana files: {indiana_files}")
        return indiana_files
    
    def _extract_from_file(self, file_path: str) -> list:
        """
        Extract structured data from a single Indiana file
    ...
    def _looks_like_candidate_data(self, df: pd.DataFrame) -> bool:
        ...
        candidate_indicators = ['candidate', 'name', 'office', 'party', 'election', 'year', 'date filed']
    ...
    def _extract_single_record(self, row: pd.Series) -> dict:
        ...
        try:
            record = {
                ...
                'county': None,  # Indiana doesn't have county info
                'district': None,  # Indiana doesn't have district info
                'address': None,  # Indiana doesn't have address info
                'city': None,  # Indiana doesn't have city info
                'state': 'Indiana',
                'zip_code': None,  # Indiana doesn't have zip info
                'phone': None,  # Indiana doesn't have phone info
                'email': None,
                'website': None,  # Indiana doesn't have email info
