#!/usr/bin/env python3
"""
Oklahoma Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only Oklahoma-specific column mapping logic remains.
"""
...
class OklahomaStructuralCleaner(BaseStructuralCleaner):
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            state_name="Oklahoma",
            state_identifiers=['oklahoma', 'ok'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (Oklahoma-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="Oklahoma"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="Oklahoma"),
