...
class OklahomaStructuralCleaner:
    """
    Oklahoma Structural Cleaner - Phase 1 of new pipeline
    ...
    def clean(self) -> pd.DataFrame:
        """
        Extract structured data from Oklahoma raw files
        
        Returns:
            pd.DataFrame: Structured data with consistent columns
        """
        logger.info("Starting Oklahoma structural cleaning")
        
        # Find Oklahoma raw files
        oklahoma_files = self._find_oklahoma_files()
        if not oklahoma_files:
            logger.warning("No Oklahoma raw files found")
            return pd.DataFrame()
        
        # Process each file and combine
        all_records = []
        
        for file_path in oklahoma_files:
            try:
        ...
        if not all_records:
            logger.warning("No records extracted from Oklahoma files")
            return pd.DataFrame()
        ...
        logger.info(f"Oklahoma structural cleaning complete: {len(df)} records")
        return df
    
    def _find_oklahoma_files(self) -> list:
        """Find all Oklahoma raw data files"""
        oklahoma_files = []
        
        if not os.path.exists(self.raw_dir):
            logger.warning(f"Raw data directory not found: {self.raw_dir}")
            return oklahoma_files
        
        # Look for Oklahoma files (case insensitive)
        for file_path in Path(self.raw_dir).rglob("*"):
            if file_path.is_file():
                filename = file_path.name.lower()
                if 'oklahoma' in filename:
                    oklahoma_files.append(str(file_path))
        
        logger.info(f"Found {len(oklahoma_files)} Oklahoma files: {oklahoma_files}")
        return oklahoma_files
    
    def _extract_from_file(self, file_path: str) -> list:
        """
        Extract structured data from a single Oklahoma file
    ...
    def _looks_like_candidate_data(self, df: pd.DataFrame) -> bool:
        ...
        candidate_indicators = ['office', 'name', 'party', 'district']
    ...
    def _extract_single_record(self, row: pd.Series) -> dict:
        ...
        try:
            record = {
                ...
                'county': None,  # Oklahoma doesn't have county info
                'district': self._extract_district(row),
                'address': None,  # Oklahoma doesn't have address info
                'city': None,  # Oklahoma doesn't have city info
                'state': 'Oklahoma',
                'zip_code': None,  # Oklahoma doesn't have zip code info
                'phone': None,  # Oklahoma doesn't have phone info
                'email': None,
                'website': None,  # Oklahoma doesn't have email info
                'filing_date': None,  # Oklahoma doesn't have filing date info
