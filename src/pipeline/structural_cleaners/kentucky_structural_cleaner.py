...
class KentuckyStructuralCleaner:
    """
    Kentucky Structural Cleaner - Phase 1 of new pipeline
    ...
    def clean(self) -> pd.DataFrame:
        """
        Extract structured data from Kentucky raw files
        
        Returns:
            pd.DataFrame: Structured data with consistent columns
        """
        logger.info("Starting Kentucky structural cleaning")
        
        # Find Kentucky raw files
        kentucky_files = self._find_kentucky_files()
        if not kentucky_files:
            logger.warning("No Kentucky raw files found")
            return pd.DataFrame()
        
        # Process each file and combine
        all_records = []
        
        for file_path in kentucky_files:
            try:
        ...
        if not all_records:
            logger.warning("No records extracted from Kentucky files")
            return pd.DataFrame()
        ...
        logger.info(f"Kentucky structural cleaning complete: {len(df)} records")
        return df
    
    def _find_kentucky_files(self) -> list:
        """Find all Kentucky raw data files"""
        kentucky_files = []
        
        if not os.path.exists(self.raw_dir):
            logger.warning(f"Raw data directory not found: {self.raw_dir}")
            return kentucky_files
        
        # Look for Kentucky files (case insensitive)
        for file_path in Path(self.raw_dir).rglob("*"):
            if file_path.is_file():
                filename = file_path.name.lower()
                if 'kentucky' in filename:
                    kentucky_files.append(str(file_path))
        
        logger.info(f"Found {len(kentucky_files)} Kentucky files: {kentucky_files}")
        return kentucky_files
    
    def _extract_from_file(self, file_path: str) -> list:
        """
        Extract structured data from a single Kentucky file
    ...
    def _looks_like_candidate_data(self, df: pd.DataFrame) -> bool:
        ...
        candidate_indicators = ['last_name', 'first_name', 'office', 'election', 'location']
    ...
    def _is_valid_candidate_row(self, row: pd.Series) -> bool:
        ...
        # Check if we have at least a last name or first name
        last_name = str(row.get('last_name', '')).strip()
        first_name = str(row.get('first_name', '')).strip()
        
        return bool(last_name and last_name != 'nan') or bool(first_name and first_name != 'nan')
    ...
    def _extract_single_record(self, row: pd.Series) -> dict:
        ...
        try:
            record = {
                ...
                'party': None,  # Kentucky doesn't have party info
                'county': None,  # Kentucky doesn't have county info
                'district': None,  # Kentucky doesn't have district info
                'address': None,  # Kentucky doesn't have address info
                'city': self._extract_city(row),
                'state': 'Kentucky',
                'zip_code': None,  # Kentucky doesn't have zip code info
                'phone': None,  # Kentucky doesn't have phone info
                'email': None,
                'website': None,  # Kentucky doesn't have email info
                'filing_date': None,  # Kentucky doesn't have filing date info
    ...
    def _extract_office(self, row: pd.Series) -> str:
        ...
        office = str(row.get('office_sought', '')).strip()
    ...
    def _extract_election_year(self, row: pd.Series) -> str:
        ...
        if pd.notna(election_date):
            ...
            if hasattr(election_date, 'strftime'):
                return election_date.strftime('%Y')
            else:
    ...
    def _extract_election_type(self, row: pd.Series) -> str:
        ...
        election_type = str(row.get('election_type', '')).strip()
        if election_type and election_type != 'nan':
