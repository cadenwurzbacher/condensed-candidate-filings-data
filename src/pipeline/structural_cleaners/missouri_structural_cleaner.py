#!/usr/bin/env python3
"""
Missouri Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only Missouri-specific column mapping logic remains.
"""
...
class MissouriStructuralCleaner(BaseStructuralCleaner):
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            state_name="Missouri",
            state_identifiers=['missouri', 'mo'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (Missouri-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="Missouri"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="Missouri"),
