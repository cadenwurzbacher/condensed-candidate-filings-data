...
class NebraskaStructuralCleaner:
    """
    Nebraska Structural Cleaner - Phase 1 of new pipeline
    ...
    def clean(self) -> pd.DataFrame:
        """
        Extract structured data from Nebraska raw files
        
        Returns:
            pd.DataFrame: Structured data with consistent columns
        """
        logger.info("Starting Nebraska structural cleaning")
        
        # Find Nebraska raw files
        nebraska_files = self._find_nebraska_files()
        if not nebraska_files:
            logger.warning("No Nebraska raw files found")
            return pd.DataFrame()
        
        # Process each file and combine
        all_records = []
        
        for file_path in nebraska_files:
            try:
        ...
        if not all_records:
            logger.warning("No records extracted from Nebraska files")
            return pd.DataFrame()
        ...
        logger.info(f"Nebraska structural cleaning complete: {len(df)} records")
        return df
    
    def _find_nebraska_files(self) -> list:
        """Find all Nebraska raw data files"""
        nebraska_files = []
        
        if not os.path.exists(self.raw_dir):
            logger.warning(f"Raw data directory not found: {self.raw_dir}")
            return nebraska_files
        
        # Look for Nebraska files (case insensitive)
        for file_path in Path(self.raw_dir).rglob("*"):
            if file_path.is_file():
                filename = file_path.name.lower()
                if 'nebraska' in filename:
                    nebraska_files.append(str(file_path))
        
        logger.info(f"Found {len(nebraska_files)} Nebraska files: {nebraska_files}")
        return nebraska_files
    
    def _extract_from_file(self, file_path: str) -> list:
        """
        Extract structured data from a single Nebraska file
    ...
    def _looks_like_candidate_data(self, df: pd.DataFrame) -> bool:
        ...
        candidate_indicators = ['office', 'name', 'party', 'address']
    ...
    def _extract_single_record(self, row: pd.Series) -> dict:
        ...
        try:
            record = {
                ...
                'county': None,  # Nebraska doesn't have county info
                'district': self._extract_district(row),
                'address': self._extract_address(row),
                'city': None,  # Nebraska doesn't have city info
                'state': 'Nebraska',
                'zip_code': None,  # Nebraska doesn't have zip code info
                'phone': self._extract_phone(row),
                'email': None,
                'website': None,  # Nebraska doesn't have email info
                'filing_date': None,  # Nebraska doesn't have filing date info
