#!/usr/bin/env python3
"""
Florida Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only Florida-specific column mapping logic remains.
"""
...
class FloridaStructuralCleaner(BaseStructuralCleaner):
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            state_name="Florida",
            state_identifiers=['florida', 'fl'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (Florida-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="Florida"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="Florida"),
