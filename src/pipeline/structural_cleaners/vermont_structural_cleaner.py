#!/usr/bin/env python3
"""
Vermont Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only Vermont-specific column mapping logic remains.
"""
...
class VermontStructuralCleaner(BaseStructuralCleaner):
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            state_name="Vermont",
            state_identifiers=['vermont', 'vt'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (Vermont-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="Vermont"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="Vermont"),
