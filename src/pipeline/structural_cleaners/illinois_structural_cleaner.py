...
class IllinoisStructuralCleaner:
    """
    Illinois Structural Cleaner - Phase 1 of new pipeline
    ...
    def clean(self) -> pd.DataFrame:
        """
        Extract structured data from Illinois raw files
        
        Returns:
            pd.DataFrame: Structured data with consistent columns
        """
        logger.info("Starting Illinois structural cleaning")
        
        # Find Illinois raw files
        illinois_files = self._find_illinois_files()
        if not illinois_files:
            logger.warning("No Illinois raw files found")
            return pd.DataFrame()
        
        # Process each file and combine
        all_records = []
        
        for file_path in illinois_files:
            try:
        ...
        if not all_records:
            logger.warning("No records extracted from Illinois files")
            return pd.DataFrame()
        ...
        logger.info(f"Illinois structural cleaning complete: {len(df)} records")
        return df
    
    def _find_illinois_files(self) -> list:
        """Find all Illinois raw data files"""
        illinois_files = []
        
        if not os.path.exists(self.raw_dir):
            logger.warning(f"Raw data directory not found: {self.raw_dir}")
            return illinois_files
        
        # Look for Illinois files (case insensitive)
        for file_path in Path(self.raw_dir).rglob("*"):
            if file_path.is_file():
                filename = file_path.name.lower()
                if 'illinois' in filename:
                    illinois_files.append(str(file_path))
        
        logger.info(f"Found {len(illinois_files)} Illinois files: {illinois_files}")
        return illinois_files
    
    def _extract_from_file(self, file_path: str) -> list:
        """
        Extract structured data from a single Illinois file
    ...
    def _looks_like_candidate_data(self, df: pd.DataFrame) -> bool:
        ...
        candidate_indicators = ['candidate', 'name', 'office', 'party', 'election', 'date filed']
    ...
    def _extract_single_record(self, row: pd.Series) -> dict:
        ...
        try:
            record = {
                ...
                'county': None,  # Illinois doesn't have county info
                'district': None,  # Illinois doesn't have district info
                'address': None,  # Illinois doesn't have address info
                'city': None,  # Illinois doesn't have city info
                'state': 'Illinois',
                'zip_code': None,  # Illinois doesn't have zip info
                'phone': None,  # Illinois doesn't have phone info
    ...
    def _extract_election_year(self, row: pd.Series) -> str:
        ...
        if election and election != 'nan':
            # Look for year pattern in election field (e.g., "GENERAL ELECTION 2024")
