#!/usr/bin/env python3
"""
Illinois Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only Illinois-specific column mapping logic remains.
"""
...
class IllinoisStructuralCleaner(BaseStructuralCleaner):
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            state_name="Illinois",
            state_identifiers=['illinois', 'il'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (Illinois-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="Illinois"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="Illinois"),
