...
class NorthDakotaStructuralCleaner:
    """
    ...
    def clean(self) -> pd.DataFrame:
        """
        Extract structured data from North Dakota raw files
        
        Returns:
            pd.DataFrame: Structured data with consistent columns
        """
        logger.info("Starting North Dakota structural cleaning")
        
        # Find North Dakota raw files
        north_dakota_files = self._find_north_dakota_files()
        if not north_dakota_files:
            logger.warning("No North Dakota raw files found")
            return pd.DataFrame()
        
        # Process each file and combine
        all_records = []
        
        for file_path in north_dakota_files:
            try:
        ...
        if not all_records:
            logger.warning("No records extracted from North Dakota files")
            return pd.DataFrame()
        ...
        logger.info(f"North Dakota structural cleaning complete: {len(df)} records")
        return df
    
    def _find_north_dakota_files(self) -> list:
        """Find all North Dakota raw data files"""
        north_dakota_files = []
        
        if not os.path.exists(self.raw_dir):
            logger.warning(f"Raw data directory not found: {self.raw_dir}")
            return north_dakota_files
        
        # Look for North Dakota files - be more specific to avoid conflicts
        for file_path in Path(self.raw_dir).rglob("*"):
            if file_path.is_file():
                filename = file_path.name.lower()
                # Only match exact North Dakota files, not files containing "nd" in other contexts
                if filename.startswith('north_dakota_') or filename.startswith('nd_'):
                    north_dakota_files.append(str(file_path))
        
        logger.info(f"Found {len(north_dakota_files)} North Dakota files: {north_dakota_files}")
        return north_dakota_files
    
    def _extract_from_file(self, file_path: str) -> list:
        """
        Extract structured data from a single North Dakota file
    ...
    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> dict:
        ...
        # Map columns based on North Dakota's actual data structure
        column_mapping = {
            'candidate_name': ['First Name', 'Middle Name', 'Last Name'],  # Will be combined
        ...
        for field in required_fields:
            if field not in record:
                if field == 'election_year':
                    ...
                    record[field] = '2024'  # Default for North Dakota
                else:
                    record[field] = None
