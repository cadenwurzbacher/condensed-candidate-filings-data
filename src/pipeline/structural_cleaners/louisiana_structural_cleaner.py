#!/usr/bin/env python3
"""
Louisiana Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only Louisiana-specific column mapping logic remains.
"""
...
class LouisianaStructuralCleaner(BaseStructuralCleaner):
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            ...
            state_identifiers=['louisiana', 'la'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (Louisiana-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="Louisiana"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="Louisiana"),
