...
class OregonStructuralCleaner:
    """
    Oregon Structural Cleaner - Phase 1 of new pipeline
    ...
    def clean(self) -> pd.DataFrame:
        """
        Extract structured data from Oregon raw files
        
        Returns:
            pd.DataFrame: Structured data with consistent columns
        """
        logger.info("Starting Oregon structural cleaning")
        
        # Find Oregon raw files
        oregon_files = self._find_oregon_files()
        if not oregon_files:
            logger.warning("No Oregon raw files found")
            return pd.DataFrame()
        
        # Process each file and combine
        all_records = []
        
        for file_path in oregon_files:
            try:
        ...
        if not all_records:
            logger.warning("No records extracted from Oregon files")
            return pd.DataFrame()
        ...
        logger.info(f"Oregon structural cleaning complete: {len(df)} records")
        return df
    
    def _find_oregon_files(self) -> list:
        """Find all Oregon raw data files"""
        oregon_files = []
        ...
        # Look for Oregon files (case insensitive)
        for file_path in Path(self.raw_dir).rglob("*"):
            if file_path.is_file():
                filename = file_path.name.lower()
                if 'oregon' in filename:
                    oregon_files.append(str(file_path))
        
        logger.info(f"Found {len(oregon_files)} Oregon files: {oregon_files}")
        return oregon_files
    
    def _extract_from_file(self, file_path: str) -> list:
        """
        Extract structured data from a single Oregon file
    ...
    def _looks_like_candidate_data(self, df: pd.DataFrame) -> bool:
        ...
        candidate_indicators = ['name', 'party', 'election', 'office', 'address']
    ...
    def _extract_single_record(self, row: pd.Series) -> dict:
        ...
        try:
            record = {
                ...
                'county': None,  # Oregon doesn't have county info
    ...
    def _extract_district(self, row: pd.Series) -> str:
        ...
        office = str(row.get('Office', '')).strip()
        if office and office != 'nan':
            # Look for district patterns like "District 1" or "1st District"
            district_match = re.search(r'(?:District|Dist\.?)\s*(\d+)', office, re.IGNORECASE)
            if district_match:
                return district_match.group(1)
            
            # Look for ordinal patterns like "1st", "2nd", "3rd"
            ordinal_match = re.search(r'(\d+)(?:st|nd|rd|th)', office, re.IGNORECASE)
            if ordinal_match:
    ...
    def _extract_city(self, row: pd.Series) -> str:
        ...
        if mailing_address and mailing_address != 'nan':
            ...
            if len(address_parts) > 1:
                ...
                if city_part and not re.search(r'\b(OR|Oregon)\b', city_part, re.IGNORECASE):
                    return city_part
        return None
    ...
    def _extract_election_year(self, row: pd.Series) -> str:
        ...
        # Try to extract from election field first
