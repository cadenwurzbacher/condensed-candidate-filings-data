...
class WashingtonStructuralCleaner(BaseStructuralCleaner):
    """
    Washington Structural Cleaner - Phase 1 of new pipeline
    ...
    def clean(self) -> pd.DataFrame:
        """
        Extract structured data from Washington raw files
        
        Returns:
            pd.DataFrame: Structured data with consistent columns
        """
        logger.info("Starting Washington structural cleaning")
        
        # Find Washington raw files
        washington_files = self._find_washington_files()
        if not washington_files:
            logger.warning("No Washington raw files found")
            return pd.DataFrame()
        
        # Process each file and combine
        all_records = []
        
        for file_path in washington_files:
            try:
        ...
        if not all_records:
            logger.warning("No records extracted from Washington files")
            return pd.DataFrame()
        ...
        logger.info(f"Washington structural cleaning complete: {len(df)} records")
        return df
    
    def _find_washington_files(self) -> list:
        """Find all Washington raw data files"""
        washington_files = []
        
        if not os.path.exists(self.raw_dir):
            logger.warning(f"Raw data directory not found: {self.raw_dir}")
            return washington_files
        
        # Look for Washington files (case insensitive)
        for file_path in Path(self.raw_dir).rglob("*"):
            if file_path.is_file():
                filename = file_path.name.lower()
                if 'washington' in filename:
                    washington_files.append(str(file_path))
        
        logger.info(f"Found {len(washington_files)} Washington files: {washington_files}")
        return washington_files
    
    def _extract_from_file(self, file_path: str) -> list:
        """
        Extract structured data from a single Washington file
    ...
    def _extract_party(self, row: pd.Series) -> str:
        ...
        party_preference = str(row.get('party_preference', '')).strip()
        if party_preference and party_preference != 'nan':
            return party_preference
        return None
    
    def _extract_county(self, row: pd.Series) -> str:
        """Extract county from row"""
        # Washington doesn't have explicit county information
        # Could be inferred from district or address if needed
        return None
    ...
    def _extract_address(self, row: pd.Series) -> str:
        ...
        mailing_address = str(row.get('mailing_address', '')).strip()
    ...
    def _extract_city(self, row: pd.Series) -> str:
        ...
        if mailing_address and mailing_address != 'nan':
            ...
            if len(address_parts) > 1:
                ...
                if city_part and not re.search(r'\b(WA|Washington)\b', city_part, re.IGNORECASE):
                    return city_part
        return None
    ...
    def _extract_election_year(self, row: pd.Series) -> str:
        ...
        # Try to extract from election field
        ...
        # Default to 2025 based on filename
        return '2025'
