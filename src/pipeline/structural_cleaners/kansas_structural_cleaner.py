#!/usr/bin/env python3
"""
Kansas Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only Kansas-specific column mapping logic remains.
"""
...
class KansasStructuralCleaner(BaseStructuralCleaner):
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            state_name="Kansas",
            state_identifiers=['kansas', 'ks'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (Kansas-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="Kansas"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="Kansas"),
