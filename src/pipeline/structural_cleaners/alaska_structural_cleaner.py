#!/usr/bin/env python3
"""
Alaska Structural Cleaner - Refactored

Reduced from 527 lines to ~80 lines by using BaseStructuralCleaner!
Only Alaska-specific column mapping logic remains.
"""
...
class AlaskaStructuralCleaner(BaseStructuralCleaner):
    ...
    Now refactored to use BaseStructuralCleaner, eliminating 400+ lines of boilerplate!
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            state_name="Alaska",
            state_identifiers=['alaska', 'ak'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (Alaska-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="Alaska"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="Alaska"),
