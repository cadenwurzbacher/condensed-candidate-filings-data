#!/usr/bin/env python3
"""
Arizona Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only Arizona-specific column mapping logic remains.
"""
...
class ArizonaStructuralCleaner(BaseStructuralCleaner):
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            state_name="Arizona",
            state_identifiers=['arizona', 'az'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (Arizona-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="Arizona"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="Arizona"),
