...
class ArizonaStructuralCleaner(BaseStructuralCleaner):
    """
    Arizona Structural Cleaner - Phase 1 of new pipeline
    ...
    def clean(self) -> pd.DataFrame:
        """
        Extract structured data from Arizona raw files
        
        Returns:
            pd.DataFrame: Structured data with consistent columns
        """
        logger.info("Starting Arizona structural cleaning")
        
        # Find Arizona raw files
        arizona_files = self._find_arizona_files()
        if not arizona_files:
            logger.warning("No Arizona raw files found")
            return pd.DataFrame()
        
        # Process each file and combine
        all_records = []
        
        for file_path in arizona_files:
            try:
        ...
        if not all_records:
            logger.warning("No records extracted from Arizona files")
            return pd.DataFrame()
        ...
        logger.info(f"Arizona structural cleaning complete: {len(df)} records")
        return df
    
    def _find_arizona_files(self) -> list:
        """Find all Arizona raw data files"""
        arizona_files = []
        
        if not os.path.exists(self.raw_dir):
            logger.warning(f"Raw data directory not found: {self.raw_dir}")
            return arizona_files
        
        # Look for Arizona files (case insensitive)
        for file_path in Path(self.raw_dir).rglob("*"):
            if file_path.is_file():
                filename = file_path.name.lower()
                if 'arizona' in filename or 'az' in filename:
                    arizona_files.append(str(file_path))
        
        logger.info(f"Found {len(arizona_files)} Arizona files: {arizona_files}")
        return arizona_files
    
    def _extract_from_file(self, file_path: str) -> list:
        """
        Extract structured data from a single Arizona file
    ...
    def _extract_single_record(self, row: pd.Series) -> dict:
        ...
        try:
            record = {
                ...
                'county': None,  # Arizona doesn't have county info
                'district': None,  # Arizona doesn't have district info
                'address': None,  # Arizona doesn't have address info
                'city': None,  # Arizona doesn't have city info
                'state': 'Arizona',
                'zip_code': None,  # Arizona doesn't have zip info
                'phone': None,  # Arizona doesn't have phone info
                'email': None,  # Arizona doesn't have email info
                'website': None,  # Arizona doesn't have website info
                'filing_date': None,  # Arizona doesn't have filing date info
    ...
    def _extract_candidate_name(self, row: pd.Series) -> str:
        """Extract candidate name from row - Arizona specific"""
    ...
    def _extract_office(self, row: pd.Series) -> str:
        """Extract office from row - Arizona specific"""
        office = str(row.get('Office', '')).strip()
    ...
    def _extract_party(self, row: pd.Series) -> str:
        """Extract party from row - Arizona specific"""
