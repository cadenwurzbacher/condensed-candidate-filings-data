#!/usr/bin/env python3
"""
North Carolina Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only North Carolina-specific column mapping logic remains.
"""
...
class NorthCarolinaStructuralCleaner(BaseStructuralCleaner):
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            ...
            state_identifiers=['north_carolina', 'nc'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (North Carolina-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="North Carolina"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="North Carolina"),
