#!/usr/bin/env python3
"""
Idaho Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only Idaho-specific column mapping logic remains.
"""
...
class IdahoStructuralCleaner(BaseStructuralCleaner):
    """
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            state_name="Idaho",
            state_identifiers=['idaho', 'id'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (Idaho-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="Idaho"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="Idaho"),
