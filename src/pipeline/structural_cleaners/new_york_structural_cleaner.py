#!/usr/bin/env python3
"""
New York Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only New York-specific column mapping logic remains.
"""
...
class NewYorkStructuralCleaner(BaseStructuralCleaner):
    """
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            state_name="New York",
            state_identifiers=['new_york', 'ny'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (New York-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="New York"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="New York"),
