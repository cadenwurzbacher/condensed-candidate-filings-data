#!/usr/bin/env python3
"""
Delaware Structural Cleaner - Refactored

Reduced from 300-500+ lines to ~80 lines by using BaseStructuralCleaner!
Only Delaware-specific column mapping logic remains.
"""
...
class DelawareStructuralCleaner(BaseStructuralCleaner):
    ...
    def __init__(self, data_dir: str = "data"):
        super().__init__(
            state_name="Delaware",
            state_identifiers=['delaware', 'de'],
            data_dir=data_dir
        )

    def _extract_record_from_row(self, row: pd.Series, file_path: str, row_idx: int) -> Optional[Dict]:
        """
        Extract a structured record from a DataFrame row (Delaware-specific).
        ...
        record = {
            ...
            'state': FieldExtractor.extract_state(row, default_state="Delaware"),
            ...
            'address_state': FieldExtractor.extract_state(row, default_state="Delaware"),
