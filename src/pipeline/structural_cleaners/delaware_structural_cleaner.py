...
class DelawareStructuralCleaner:
    ...
    def clean(self) -> pd.DataFrame:
        ...
        """
        logger.info("Starting Delaware structural cleaning")
        
        # Find Delaware raw files
        delaware_files = self._find_delaware_files()
        if not delaware_files:
            logger.warning("No Delaware raw files found")
            return pd.DataFrame()
        
        # Process each file and combine
        all_records = []
        
        for file_path in delaware_files:
            try:
        ...
        if not all_records:
            logger.warning("No records extracted from Delaware files")
            return pd.DataFrame()
        ...
        logger.info(f"Delaware structural cleaning complete: {len(df)} records")
        return df
    
    def _find_delaware_files(self) -> list:
        """Find all Delaware raw data files"""
        delaware_files = []
        
        if not os.path.exists(self.raw_dir):
            logger.warning(f"Raw data directory not found: {self.raw_dir}")
            return delaware_files
        
        # Look for Delaware files (case insensitive)
        for file_path in Path(self.raw_dir).rglob("*"):
            if file_path.is_file():
                filename = file_path.name.lower()
                if 'delaware' in filename or 'de' in filename:
                    delaware_files.append(str(file_path))
        
        logger.info(f"Found {len(delaware_files)} Delaware files: {delaware_files}")
        return delaware_files
    
    def _extract_from_file(self, file_path: str) -> list:
        """
        Extract structured data from a single Delaware file
    ...
    def _extract_single_record(self, row: pd.Series) -> dict:
        ...
        try:
            record = {
                ...
                'party': None,  # Delaware doesn't have party info
                'county': self._extract_county(row),
                'district': self._extract_district(row),
                'address': None,  # Delaware doesn't have address info
                'city': None,  # Delaware doesn't have city info
                'state': 'Delaware',
                'zip_code': None,  # Delaware doesn't have zip info
                'phone': self._extract_phone(row),
                'email': None,
                'website': None,  # Delaware doesn't have email info
    ...
    def _extract_election_type(self, row: pd.Series) -> str:
        ...
        if election and election != 'nan':
            # Parse election type from election field
