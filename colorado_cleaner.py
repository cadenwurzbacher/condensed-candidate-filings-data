...
class ColoradoCleaner:
    ...
    def _process_office_and_district(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Clean up decimal places and add underscore to prevent pandas auto-conversion to float
        df['district_cleaned'] = df['district_cleaned'].apply(lambda x: f"_{x.split('.')[0]}" if x and x != 'nan' else "")
    ...
    def _add_required_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['original_filing_date'] = None  # Not available in Colorado data
        ...
        if 'district_cleaned' in df.columns:
            # Convert district to string to match Alaska's format, handling NaN values
