...
def scrape_arizona_vote():
    base_url = "https://apps.arizona.vote/electioninfo"
    election_url = f"{base_url}/Election/47"
    
    print("Setting up scraper with enhanced anti-detection...")
    
    # Create a more robust scraper with better headers
    ...
    try:
        ...
        response = scraper.get(election_url)
        response.raise_for_status()
        ...
        filename = os.path.join(raw_dir, 'arizona_candidates.xlsx')
